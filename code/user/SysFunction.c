...
void ShowTemperature(void)
{
	...
	//DS18b20 温度测试 
		...
		if( ds18b20_count >= 200)
		{
			...
			{
				tmpperatureDS *= -1;
				sprintf(TempDisplayBuf,"T:-%d.%02d",tmpperatureDS/16,(10000/(tmpperatureDS&0xf))/100);
			}
			else
			{
				sprintf(TempDisplayBuf,"T:%d.%02d",tmpperatureDS/16,(10000/(tmpperatureDS&0xf))/100);
		...
		if( mlx96014_count >= 200)
		{
			...
			sprintf(TempDisplayBuf,"T:%d.%02d",tmpperatureMLX/100,tmpperatureMLX%100);
...
void SendDataToPc(uint8_t * DataBuf,uint8_t dataLen)
{
	...
	if(Function_Select == FUNCTION_ECG )
	{
		DataBuf[dataLen+9] = 0x31; 
	}
	else if(Function_Select == FUNCTION_PPG )
	{
		DataBuf[dataLen+9] = 0x32; 
	}
	else if(Function_Select == FUNCTION_GSR )
	{
		DataBuf[dataLen+9] = 0x33; 
	}
	else
	{
		DataBuf[dataLen+9] = 0x34; 
	}
...
void Caculate_HR_SpO2(uint8_t * DataBuf,uint8_t dataLen)
{
  ...
  uint32_t un_temp;
	...
	for( j = 0; j< dataLen; j ++)
	{
		...
		if(un_min>aun_red_buffer[ir_red_data_count])
       un_min=aun_red_buffer[ir_red_data_count];    //update signal min
    if(un_max<aun_red_buffer[ir_red_data_count])
       un_max=aun_red_buffer[ir_red_data_count];    //update signal max
...
void ShowHeart(u8 show)
{
	if( show == 1 )
	{
		OLED_DrawBMP(0,6,16,8,BMP_Buf);
	}
	else
	{
		OLED_ClearBMP(0,6,16,8);
	} 
}
